...
3. **回溯策略**
   - 当逻辑推理无法继续时，进入猜测阶段
   - 首先尝试 `guess_bivalue_in_cell`：
     * 优先选择只有两个可能值的单元格（这些位置在寻找裸单元格时已被记录）
     * 保存当前状态
     * 尝试第一个可能值
     * 每次猜测后都会重新应用逻辑推理策略（锁定候选数和裸单元格）
     * 如果失败，回退并尝试第二个可能值
   - 如果没有找到双值单元格，则使用 `guess_some_cell`：
     * 检查最多3个未解决的单元格
     * 选择候选数最少的那个单元格
     * 尝试所有可能的数字
     * 每次猜测后都会重新应用逻辑推理策略
...
4. **编译器优化**
   - 使用 `unroll` 宏展开关键循环
   - 避免不可预测的分支，提高分支预测准确率
   - 使用 `UncheckedIndexArray` 优化数组访问，减少边界检查开销
